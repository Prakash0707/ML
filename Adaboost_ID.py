...
class AdaboostClassifer:
    ...
    def fit(self, X, y):
        num_of_samp, num_of_feat = X.shape

        weight_feat = np.full(num_of_samp, (1 / num_of_samp))
        ...
        for _ in range(2):
            ...
            for feature_index in range(num_of_feat):
                ...
                for t in thresholds_array:
                    ...
                    predictions = np.ones(num_of_samp)
                    predictions[X_column < t] = -1

                    misclassified = weight_feat[y != predictions]
                    ...
                    if weights_of_misclassfied_instance < minimum_error:
                        cla.polarity = polarity_level
                        cla.threshold_point = t
                        cla.feature_index = feature_index
            ...
            predicted_values = cla.predict(X)

            weight_feat *= np.exp(-cla.alpha * y * predicted_values)
            weight_feat /= np.sum(weight_feat)

            self.cla.append(cla)

    def predict(self, X):
        cla_preds = [cla.alpha * cla.predict(X) for cla in self.cla]
        y_pred = np.sum(cla_preds, axis=0)
...
class Id3_deci:
    ...
    def predict(self, X):
        num_of_samp = X.shape[0]
        X_column = X[:, self.feature_index]
        predicted_values = np.ones(num_of_samp)
        if self.polarity == 1:
    ...
    if dataset_name == "car":
        ...
        for i in range(10):
            ...
            for train_index, test_index in kfold_adaboost.split(X):
                ...
                predicted_value = adaboost_model.predict(X_test)

                accuracy_dataset = accuracy(predicted_value, y_test)
